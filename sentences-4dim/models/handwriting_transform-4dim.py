...
class HandwritingTransformer(nn.Module):
    ...
    def forward(self, text_ids, stroke_seq, src_key_padding_mask=None, tgt_key_padding_mask=None):
        ...
        pos_text = torch.arange(
            0, T_text, device=device).unsqueeze(0).repeat(B, 1)
        text_emb = self.text_embedding(text_ids) + self.pos_encoder(pos_text)
        memory = self.text_encoder(
            text_emb, src_key_padding_mask=src_key_padding_mask)

        # --- Decoder ---
        pos_stroke = torch.arange(
            0, T_stroke, device=device).unsqueeze(0).repeat(B, 1)
        stroke_emb = self.stroke_embedding(
            stroke_seq) + self.pos_decoder(pos_stroke)
        tgt_mask = nn.Transformer.generate_square_subsequent_mask(
            T_stroke, device=device)  # UserWarningの原因だが動作は正常
        ...
        pi_logits, mu_x, mu_y, sigma_x_log, sigma_y_log, rho_xy_tanh = mdn_params.split(
            self.n_mixtures, dim=-1)
...
def mdn_loss(pi, mu_x, mu_y, sigma_x, sigma_y, rho_xy, target_dxdy):
    ...
    log_pdf = -z / (2 * one_minus_rho_sq) - torch.log(2 *
                                                      math.pi * sigma_x * sigma_y * torch.sqrt(one_minus_rho_sq))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for text_ids, seq, text_padding_mask, seq_padding_mask in dataloader:
            text_ids, seq, text_padding_mask, seq_padding_mask = \
                text_ids.to(device), seq.to(device), text_padding_mask.to(
                    device), seq_padding_mask.to(device)
...
def generate_strokes(model, text, char2id, id2char, max_len=700, device="cuda"):
    ...
    with torch.no_grad():
        text_ids = [char2id["<s>"]] + \
            [char2id.get(c, char2id["<unk>"])
             for c in text] + [char2id["</s>"]]
...
def plot_strokes(strokes, title="Generated Handwriting"):
    ...
    for (x, y, end) in strokes:
        ...
        if end == 0:
            ...
            current_stroke_x, current_stroke_y = [], []
    if current_stroke_x:
        ax.plot(current_stroke_x, current_stroke_y, "k-", linewidth=2.5)
...
def collate_fn(batch, text_pad_id):
    ...
    padded_texts = pad_sequence(
        texts, batch_first=True, padding_value=text_pad_id)
    ...
    stroke_padding_mask = torch.arange(padded_strokes.size(1))[
        None, :] >= stroke_lengths[:, None]
...
def save_strokes_to_json(strokes, text, directory="outputs"):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        json.dump({"text": text, "strokes": strokes},
...
def plot_from_file(filepath):
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            data = json.load(f)
        plot_strokes([tuple(p) for p in data["strokes"]],
                     title=f"Generated for '{data['text']}' (from file)")
    except Exception as e:
        print(f"An error occurred while plotting from file: {e}")
...
if __name__ == "__main__":
    ...
    DROPOUT, EPOCHS, LR, BATCH_SIZE = 0.1, 300, 0.0005, 8
    ...
    dataloader = DataLoader(dataset, batch_size=BATCH_SIZE,
                            shuffle=True, collate_fn=collate_with_pad)
    ...
    trained_model = train_model(
        model, dataloader, epochs=EPOCHS, lr=LR, device=device)
    print("--- Finish Training ---")

    test_text = "うい" if 'う' in char2id and 'い' in char2id else (
        unique_chars[0] if unique_chars else "a")
    print(f"\nGenerating for text: '{test_text}'")

    generated_strokes = generate_strokes(
        trained_model, test_text, char2id, id2char, device=device)
