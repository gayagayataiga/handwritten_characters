...
class HandwritingTransformer(nn.Module):
    def __init__(self, vocab_size, d_model=256, nhead=8,
        ...
        # ▼▼▼▼▼ ここから修正 ▼▼▼▼▼
        # self.stroke_in の名前を self.stroke_embed に変更
        self.stroke_embed = nn.Linear(stroke_dim, d_model) 
        
        # 結合された入力 (stroke_embed + memory) を d_model に射影する層を追加
    ...
    def decode(self, strokes, stroke_mask, memory, memory_mask):
        ...
        # ▼▼▼▼▼ ここから修正 ▼▼▼▼▼
        # 1. エンコーダーからの文脈ベクトルを取得 (文字IDは1つなので L=1)
        # memory の形状: [B, 1, D_model] -> context_vec の形状: [B, D_model]
        context_vec = memory.squeeze(1)
        
        # 2. 文脈ベクトルをストロークの系列長 T に合わせて拡張
        # [B, D_model] -> [B, 1, D_model] -> [B, T, D_model]
        context_vec_expanded = context_vec.unsqueeze(1).expand(-1, T, -1)
        
        # 3. ストロークデータを埋め込み
        stroke_embedding = self.stroke_embed(strokes)
        
        # 4. ストローク埋め込みと文脈ベクトルを結合！
        decoder_input = torch.cat([stroke_embedding, context_vec_expanded], dim=-1)
        
        # 5. 結合したベクトルを元のモデル次元 D_model に戻す
...
if __name__ == "__main__":
    ...
    train_loop("sentences-4dim/oneletters/resampling", vocab, epochs=5, batch_size=512, lr=1e-4)
