...
def sample_bivariate(pi, mu_x, mu_y, sigma_x, sigma_y, rho):
    ...
    cov = [
        [sigma_x[comp].item()**2, rho[comp].item()*sigma_x[comp].item()*sigma_y[comp].item()],
        [rho[comp].item()*sigma_x[comp].item()*sigma_y[comp].item(), sigma_y[comp].item()**2]
    ]
...
def generate_strokes(model, vocab, text="あ", max_steps=200, device="cuda"):
    ...
    strokes = [torch.tensor([[0.0, 0.0, 1.0, 0.0]], dtype=torch.float32, device=device)]
    ...
    for _ in range(max_steps):
        ...
        pen_state = torch.multinomial(torch.softmax(pen_logits, dim=-1), 1).item()
        pen_onehot = [1, 0] if pen_state == 0 else [0, 1]
        new_step = torch.tensor([[dx, dy] + pen_onehot], dtype=torch.float32, device=device)
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("sentences-4dim/oneletters/checkpoints/handwriting_epoch10.pt", map_location=device))
